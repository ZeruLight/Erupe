...
func cleanupDiva(s *Session) {
	...
	db.Exec("DELETE FROM events WHERE event_type='diva'")
}
...
func generateDivaTimestamps(s *Session, start uint32, debug bool) []uint32 {
	...
	if start == 0 || gametime.TimeAdjusted().Unix() > int64(start)+2977200 {
		...
		db.Exec("INSERT INTO events (event_type, start_time) VALUES ('diva', to_timestamp($1)::timestamp without time zone)", start)
	}
	timestamps[0] = start
	timestamps[1] = timestamps[0] + 601200
	timestamps[2] = timestamps[1] + 3900
...
func handleMsgMhfGetUdSchedule(s *Session, db *sqlx.DB, p mhfpacket.MHFPacket) {
	pkt := p.(*mhfpacket.MsgMhfGetUdSchedule)
	bf := byteframe.NewByteFrame()
	id, start := uint32(0xCAFEBEEF), uint32(0)
	rows, _ := db.Queryx("SELECT id, (EXTRACT(epoch FROM start_time)::int) as start_time FROM events WHERE event_type='diva'")
	for rows.Next() {
		rows.Scan(&id, &start)
	}
	...
	if config.GetConfig().DebugOptions.DivaOverride >= 0 {
		if config.GetConfig().DebugOptions.DivaOverride == 0 {
			if config.GetConfig().ClientID >= config.Z2 {
				s.DoAckBufSucceed(pkt.AckHandle, make([]byte, 36))
			} else {
...
func handleMsgMhfGetUdInfo(s *Session, db *sqlx.DB, p mhfpacket.MHFPacket) {
...
func handleMsgMhfGetKijuInfo(s *Session, db *sqlx.DB, p mhfpacket.MHFPacket) {
...
func handleMsgMhfSetKiju(s *Session, db *sqlx.DB, p mhfpacket.MHFPacket) {
