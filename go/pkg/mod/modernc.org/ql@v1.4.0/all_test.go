...
func TestIssue224(t *testing.T) {
	...
	defer os.RemoveAll(dir)
	pth := filepath.Join(dir, "ql.db")
	db, err := sql.Open("ql", pth+"?removeemptywal=1")
	if err != nil {
		t.Fatal(err)
	}
...
// https://gitlab.com/cznic/ql/-/issues/225
func TestGroupByRaceCondition(t *testing.T) {
	...
	if _, err = tx.Exec(`CREATE TABLE artists (name string)`); err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 100; i++ {
		...
		if _, err = tx.Exec(`INSERT INTO artists (name) VALUES($1)`, fmt.Sprintf("artist-%d", i%5)); err != nil {
			t.Fatal(err)
		}
